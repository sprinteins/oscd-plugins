...
We are following the [trunk-based development rules](https://trunkbaseddevelopment.com/): one trunk (main), short-lived feature branches that should not live longer than a sprint. 
...
* When you are assigned to an issue from the Sprint Backlog, please do the following:
	* Create a new branch following this convention:
		* `{type}/{scope}/{feature-name}` with :
			...
			* `scope`: plugin name (can be shortened AD=Auto-doc) | `monorepo`
			* `feature-name`: name of the feature (corresponding to the User Story)
