...
describe('imageUtils', () => {
    describe('convertImageToBase64', () => {
        ...
        it('should reject with an error if the file cannot be read', async () => {
            ...
            await expect(imageUtils.convertImageToBase64(file)).rejects.toThrow('Error reading file');
        ...
        it('should reject with an error if the result is not a string', async () => {
            ...
            FileReader.prototype.readAsDataURL = function() {
                if (this.onload) {
                    ...
                    Object.defineProperty(event, 'target', { value: { result: null } });
                    this.onload(event as ProgressEvent<FileReader>);
                }
            };

            await expect(imageUtils.convertImageToBase64(file)).rejects.toThrow('Failed to convert image to Base64');
