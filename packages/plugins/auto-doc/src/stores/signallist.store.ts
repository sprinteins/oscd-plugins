...
function getSignalList(
	...
	const contexts = Array.from(ieds)
		.flatMap((ied) => {
...
function fillMessagePublisherData(context: SourceTarget): MessagePublisher {
	...
	const voltageLevel =
		lNode?.closest('VoltageLevel')?.getAttribute('name') ?? ''
...
function getDataObjectInformation(
	...
	const dataTypeLeaf = queryDataTypeLeaf(
		dataTypeTemplates,
		lnType,
		doSegments,
		daSegments
...
function handleIED(targetContext: TargetContext) {
	const accessPoints = targetContext.ied.querySelectorAll(
		':scope > AccessPoint'
	)
...
function handleAccessPoint(targetContext: TargetContext) {
	const lDevices = targetContext.accessPoint.querySelectorAll(
		':scope > Server > LDevice'
	)
...
function handleLn(targetContext: TargetContext) {
	const extRefs = targetContext.ln.querySelectorAll(
		':scope > Inputs > ExtRef'
	)

	return Array.from(extRefs).map((extRef) => {
		const newTargetContext = {
			...targetContext,
			extRef
		}
...
function getSourceContext(targetContext: TargetContext): SourceContext | null {
	...
	const sourceLN = sourceLDevice.querySelector(
...
//==== PUBLIC ACTIONS
function getPublishingLogicalDevices(
	filter: MessagePublisherFilter = {},
	subscriberFilter: MessageSubscriberFilter = {}
): { pdfRows: PdfRowStructure[]; invaliditiesReports: InvalditiesReport[] } {
...
function filterSubscriber(
	...
	return pdfRows.map((pdfRow) => {
		const gooseSubscribers = gooseFilter
			? pdfRow.matchedSubscribers[SignalType.GOOSE].filter((s) =>
					doesNameMatchFilter(s, gooseFilter)
				)
...
function groupByPublisher(pdfRows: PdfRowStructure[]): PdfRowStructure[] {
	...
	for (const pdfRow of pdfRows) {
		...
		const existingPublisher = groupedPdfRows.find(
			(r) => getPublisherIdentifier(r) === publisherId
		)
...
function filterMessagePublishers(
	messagePublishers: MessagePublisher[],
	filter: MessagePublisherFilter
): PdfRowStructure[] {
	const matchedValueAndCorrespondingPublisher: PdfRowStructure[] = []
	const allMessagePublishers: MessagePublisher[] = []

	const filterSearchKeys = Object.keys(
		filter
	) as (keyof MessagePublisherFilter)[]
	for (const publisher of messagePublishers) {
		...
		for (const searKey of filterSearchKeys) {
			...
			if (substringSearch !== undefined) {
				const publisherValueFromSearchKey: string =
					getValueFromNestedProperty(publisher, searKey)
				if (
					publisherValueFromSearchKey
						.toLocaleLowerCase()
						.includes(substringSearch.toLocaleLowerCase()) ||
					substringSearch.trim() === ''
				) {
...
function getValueFromNestedProperty(
	publisher: MessagePublisher,
	key: keyof MessagePublisherFilter
): string {
	...
	const value: string = path
		.split('.')
		.reduce((pub, k) => (pub ? pub[k] : ''), publisher) as unknown as string
	return value
}
