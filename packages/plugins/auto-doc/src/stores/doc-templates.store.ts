...
function createAutoDocArea(xmlDocument: Document): Element {
	...
	const existingAutoDocArea = rootElement.querySelector(
		'Private[type="AUTO_DOC"]'
	)
	if (existingAutoDocArea) {
		return existingAutoDocArea
	}
	const newPrivateArea = createElement(xmlDocument, 'Private', {
		type: 'AUTO_DOC'
	})
...
function getAllDocumentTemplates() {
	...
	if (currentPrivateArea) {
		return Array.from(
			currentPrivateArea.querySelectorAll('DocumentTemplate')
		)
	}
	return []
}
...
function importDocumentTemplates(docTemplates: Element[]) {
	...
	const importedDocTemplates = docTemplates.map((docTemplate) =>
		xmlDoc.importNode(docTemplate, true)
	)

	eventStore.createMultipleAndDispatchActionEvent(
		currentPrivateArea,
		importedDocTemplates,
		'Import auto doc templates'
	)
}

function editDocumentTemplateTitleAndDescription(
...
function addBlockToDocumentTemplate(
	docTemplate: Element,
	type: ElementType,
...
function editBlockContentOfDocumentTemplate(
	docTemplate: Element,
	blockId: string,
	content: string
) {
	...
	if (blockElement) {
		...
		eventStore.updateAndDispatchActionEvent(blockElement, {
			id: blockIdAttribute,
...
function moveBlockInDocumentTemplate(
	docTemplate: Element,
	blockId: string,
	reference?: Element | null
) {
	...
	if (blockIdElement) {
		eventStore.moveAndDispatchActionEvent(
			docTemplate,
			blockIdElement,
			reference
		)
...
function deleteBlockFromDocumentTemplate(
	docTemplate: Element,
	blockId: string
) {
	const blockElement = docTemplate.querySelector(`Block[id="${blockId}"]`)
...
function duplicateBlockFromDocumentTemplate(
	docTemplate: Element,
	blockId: string,
	position: number
) {
...
function duplicateDocumentTemplate(docTemplateId: string) {
	...
	if (currentPrivateArea) {
		...
		if (docTemplate) {
			...
			newDocTemplate.setAttribute(
				'description',
				`Copied from ${title}'s description: ${description}`
			)
...
function setMasterTemplateFlag(isMaster: boolean) {
	...
	const updates: { masterTemplate: string; type: string } = {
