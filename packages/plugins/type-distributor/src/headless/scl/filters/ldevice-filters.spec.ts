...
describe('filterByLDevice', () => {
			...
			accessPoints: [
				{
					...
					name: 'AP1',
					lNodes: [
						{ lnClass: 'XCBR', lnType: 'XCBR_Type1', lnInst: '1', lDeviceName: 'LD_Circuit_Breaker' },
						{ lnClass: 'XSWI', lnType: 'XSWI_Type1', lnInst: '1', lDeviceName: 'LD_Disconnector' }
					]
				},
				{
					element: document.createElement('AccessPoint'),
					name: 'AP2',
					lNodes: [
						{ lnClass: 'MMXU', lnType: 'MMXU_Type1', lnInst: '1', lDeviceName: 'LD_Measurement' }
					]
				}
			]
		},
			...
			accessPoints: [
				{
					...
					name: 'AP_Main',
					lNodes: [
						{ lnClass: 'CSWI', lnType: 'CSWI_Type1', lnInst: '1', lDeviceName: 'LD_Control_Unit' },
			...
			accessPoints: [
				{
					...
					name: 'AP_Test',
					lNodes: [
						{ lnClass: 'LLN0', lnType: 'LLN0_Type', lnInst: '', lDeviceName: undefined }
					]
				}
			]
		}
	...
	it('GIVEN IEDs with matching LDevice names WHEN filtering by LDevice THEN should return IED with matching LNodes only', () => {
		...
		expect(result[0].accessPoints[0].lNodes[0].lDeviceName).toBe('LD_Circuit_Breaker')
	})
	...
	it('GIVEN IED with LNodes in multiple AccessPoints WHEN filtering THEN should preserve parent AccessPoint hierarchy', () => {
		...
		expect(result[0].accessPoints[0].lNodes[0].lDeviceName).toBe('LD_Measurement')
	})
	...
	it('GIVEN multiple IEDs with matching LDevice names WHEN filtering THEN should return all matching IEDs', () => {
		...
		expect(result[0].accessPoints[0].lNodes).toHaveLength(1)
		expect(result[0].accessPoints[0].lNodes[0].lDeviceName).toBe('LD_Control_Unit')
	})
	...
	it('GIVEN IEDs WHEN filtering with case-insensitive term THEN should match correctly', () => {
		...
		expect(result).toHaveLength(1)
		expect(result[0].accessPoints[0].lNodes[0].lDeviceName).toBe('LD_Disconnector')
	})
	...
	it('GIVEN IEDs with undefined LDevice names WHEN filtering THEN should handle undefined gracefully', () => {
		...
		// THEN should not match undefined lDeviceNames
		expect(result).toHaveLength(0)
	})
	...
	it('GIVEN IED with AccessPoint containing only non-matching LNodes WHEN filtering THEN should not include IED', () => {
		// GIVEN IED with only undefined lDeviceName
