...
export type MatchingContext = { scdBay: Element; bayType: BayType }
...
export function resolveMatchingContext({
	...
	bayTypes,
	scdBay
}: FindMatchingEquipmentParams): MatchingContext {
	if (!selectedBayTypeUuid) throw new Error('No BayType selected')
	const bayType = bayTypes.find((bay) => bay.uuid === selectedBayTypeUuid)
	if (!bayType) throw new Error(`BayType "${selectedBayTypeUuid}" not found`)
	if (!scdBay) throw new Error('No Bay selected in SCD')
