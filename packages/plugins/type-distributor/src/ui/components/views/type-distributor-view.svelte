<script lang="ts">
    ...
    const functionTemplates = $derived((bayTypeWithTemplates?.functionTemplates ?? []).filter((t): t is NonNullable<typeof t> => t != null));
    const conductingEquipmentTemplates = $derived((bayTypeWithTemplates?.conductingEquipmentTemplates ?? []).filter((t): t is NonNullable<typeof t> => t != null));
</script>
