...
import { generateUniqueId } from "./headless/utils";
...
export function buildObjectTree() {
	...
	for (const lDevice of lDevices) {
		const device: TreeNode = { id: generateUniqueId(), name: lDevice.getAttribute("inst") || "", type: NODE_TYPE.logicalDevice, isOpen: true, children: [] }

		const lNodes = Array.from(lDevice.querySelectorAll("LN"))

		for (const lNode of lNodes) {
			const node: TreeNode = { id: generateUniqueId(), name: lNode.getAttribute("lnClass") || "", type: NODE_TYPE.logicalNode, isOpen: true, children: [] }

			const dObjects = Array.from(lNode.querySelectorAll("DOI"))

			for (const dObject of dObjects) {
				const object: TreeNode = { id: generateUniqueId(), name: dObject.getAttribute("name") || "", type: NODE_TYPE.dataObjectInstance }
