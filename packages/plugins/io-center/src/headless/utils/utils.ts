	...
	return store.connections.some(
		(connection) =>
			(connection.from.name === fromNode &&
				...
				_.isEqual(connection.from.port, fromPort) &&
				_.isEqual(connection.to.port, toPort)) ||
			(connection.from.name === toNode &&
				connection.to.name === fromNode &&
				_.isEqual(connection.from.port, toPort) &&
				_.isEqual(connection.to.port, fromPort))
	)
}

export function isDoToLcConnectionAllowed(
	...
	toast.push(
		`Data Object with cdc (${cdc}) can not be linked to a Logical Conditioner of type ${lc.type}!`
	)
	return false
}
...
export async function getCoordinates(connectionPoint: ConnectionPoint) {
	...
	const portNumber =
		connectionPoint.port.index || connectionPoint.port.index === 0
			? `${connectionPoint.port.index}`
			: ''
