...
export function stopDrawing(
	...
	if (startCircle && target && startCircle !== target) {
		...
		if (
			startCircleIndex !== null &&
			startNodeType &&
			!connectionExists(fromNode, startCircleIndex, toNode, index)
		) {
			const connection = {
				id: `${fromNode}-${startCircleIndex}-${toNode}-${index}`,
				from: {
					name: fromNode,
					index: startCircleIndex,
					type: startNodeType
				},
...
export async function getCoordinates(connectionPoint: ConnectionPoint) {
	...
	const circle = target.querySelector(
		`#${connectionPoint.name.includes('left') ? 'left' : 'right'}-circle-${connectionPoint.index}`
	)
