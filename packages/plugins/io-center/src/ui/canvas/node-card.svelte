...
function unselectCurrentCard() {
	if (!logicalKind)
		iedStore.selectedDataObjectIds = iedStore.selectedDataObjectIds.filter(
			(id) => id !== node.id
		)
	else logicalStore.unselectLogical(logicalKind, node.id)
}
</script>
