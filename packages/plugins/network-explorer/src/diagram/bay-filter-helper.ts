...
export function filterNodesAndEdgesForBay(
	...
	const filteredConnectedIedIds = excludeIedsBelongingToOtherBays(connectedIedIds, targetBayName, iedBayMap)
	const allowedIedIds = combineAllowedIedIds(iedIdsInTargetBay, filteredConnectedIedIds)

	const filteredNodes = filterNodesForTargetBay(nodes, targetBayName, allowedIedIds)
...
function doesTargetBayNodeExist(targetBayName: string, nodes: FlowNodes[]): boolean {
	return nodes.some(n => n.id === `${BAY_ID_PREFIX}${targetBayName}`)
}

function findConnectedIedIds(edges: Edge[], iedIdsInTargetBay: Set<string>): Set<string> {
...
function excludeIedsBelongingToOtherBays(
	...
	for (const iedId of connectedIedIds) {
		...
		const belongsToOtherBay = doesIedBelongToOtherBay(iedName, targetBayName, iedBayMap)
...
function filterEdgesBetweenAllowedIeds(edges: Edge[], allowedIedIds: Set<string>): Edge[] {
