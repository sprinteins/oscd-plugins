...
export class UCNetworkInformation {
	...
	public gatherNetworkings(): Networking[] {
		...
		for (const ap of connectedAPs) {
			...
			const physConnections = this.extractPhysConnectionsFromAP(
				ap.element
			)
	...
	private extractAddressFromAP(apElement: Element): Address {
		const ip =
			this.scdQueries.searchConnectedAPIP({ root: apElement })?.element
				?.innerHTML ?? ''
		const ipSubnet =
			this.scdQueries.searchConnectedAPIPSubnet({ root: apElement })
				?.element?.innerHTML ?? ''
		const ipGateway =
			this.scdQueries.searchConnectedAPIPGateway({ root: apElement })
				?.element?.innerHTML ?? ''
	...
	private extractPhysConnectionsFromAP(apElement: Element): PhysConnection[] {
		const connectionElements =
			this.scdQueries.seachConnectedPhysConnections({ root: apElement })
		const physConnections = connectionElements.map((physConnection) => {
			const cable =
				this.scdQueries.seachPhysConnectionCable({
					root: physConnection.element
				})?.element.innerHTML ?? ''
			const port =
				this.scdQueries.seachPhysConnectionPort({
					root: physConnection.element
				})?.element.innerHTML ?? ''
			const type =
				this.scdQueries.seachPhysConnectionType({
					root: physConnection.element
				})?.element.innerHTML ?? ''
			const plug =
				this.scdQueries.seachPhysConnectionPlug({
					root: physConnection.element
				})?.element.innerHTML ?? ''
	...
	public extractPhysConnectionCable(
		physConnElement: Element
	): SCDElement | null {
		const cableElement = this.scdQueries.seachPhysConnectionCable({
			root: physConnElement
		})
		return cableElement
	}
	...
	private enrichNetworkingWithConnectedIEDs(networkings: Networking[]): void {
		for (const networking of networkings) {
			const connectedNetworking = this.findConnectedNetworkingByCableName(
				networking,
				networkings
			)
			...
			if (connectedNetworking.length !== 1) {
				console.log({
					level: 'debug',
					msg: 'could not match networking exaclty, so no cable will be shown',
					ied: networking.iedName
				})
	...
	private findConnectedNetworkingByCableName(
		srcNetworking: Networking,
		...
		const connectedNetworking = networkings.filter((otherNetworking) => {
			...
			const hasSameCableName =
				srcNetworking.cable === otherNetworking.cable
