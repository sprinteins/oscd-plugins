...
function describeEnumType(element: Element) {
	...
	for (const val of Array.from(element.children)
		...
		.sort((v1, v2) => {
			...
			return (
				parseInt(
					(_a = v1.getAttribute('ord')) !== null && _a !== void 0
						? _a
						: '',
					10
				) -
				parseInt(
					(_b = v2.getAttribute('ord')) !== null && _b !== void 0
						? _b
						: '',
					10
				)
		...
		vals[
			(_a = val.getAttribute('ord')) !== null && _a !== void 0 ? _a : ''
		] = (_b = val.textContent) !== null && _b !== void 0 ? _b : ''
...
function describeDAType(element: Element) {
	...
	for (const bda of Array.from(element.children)
		...
		bdas[
			(_a = bda.getAttribute('name')) !== null && _a !== void 0 ? _a : ''
...
function describeDOType(element: Element) {
	...
	for (const sdo of Array.from(element.children)
		...
		.sort((c1, c2) => c1.outerHTML.localeCompare(c2.outerHTML))) {
		const [name, type, transient] = ['name', 'type', 'transient'].map(
			(attr) => sdo.getAttribute(attr)
		)
...
function describeLNodeType(element: Element) {
	...
	for (const doElement of Array.from(element.children)
		...
		.sort((c1, c2) => c1.outerHTML.localeCompare(c2.outerHTML))) {
		const [name, type, transient] = ['name', 'type', 'transient'].map(
			(attr) => doElement.getAttribute(attr)
		)
...
export function describeElement(element: Element) {
	let _a, _b
	return (_b =
		(_a = typeDescriptions[element.tagName]) === null || _a === void 0
			? void 0
			: _a.call(typeDescriptions, element)) !== null && _b !== void 0
		? _b
		: { xml: element.outerHTML }
}
